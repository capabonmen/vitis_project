...
typedef struct {
    ...
    XConvex_hull_accel_uio_map maps[ MAX_UIO_MAPS ];
} XConvex_hull_accel_uio_info;

/***************** Variable Definitions **************************************/
static XConvex_hull_accel_uio_info uio_info;
...
static int uio_info_read_name(XConvex_hull_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XConvex_hull_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XConvex_hull_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XConvex_hull_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XConvex_hull_accel_Initialize(XConvex_hull_accel *InstancePtr, const char* InstanceName) {
	XConvex_hull_accel_uio_info *InfoPtr = &uio_info;

...
entity convex_hull_accel_control_s_axi is
generic (
...
end entity convex_hull_accel_control_s_axi;
...
architecture behave of convex_hull_accel_control_s_axi is
    ...
    constant ADDR_HULL_SIZE_DATA_0 : INTEGER := 16#20#;
    constant ADDR_HULL_SIZE_CTRL   : INTEGER := 16#24#;
    ...
    signal int_hull_size_ap_vld : STD_LOGIC;
    signal int_hull_size       : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_HULL_SIZE_DATA_0 =>
                        rdata_data <= RESIZE(int_hull_size(31 downto 0), 32);
                    when ADDR_HULL_SIZE_CTRL =>
                        rdata_data(0) <= int_hull_size_ap_vld;
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_hull_size <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (hull_size_ap_vld = '1') then
                    int_hull_size <= UNSIGNED(hull_size);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_hull_size_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (hull_size_ap_vld = '1') then
                    int_hull_size_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_HULL_SIZE_CTRL) then
                    int_hull_size_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;

...
// ==============================================================
`timescale 1ns/1ps
module convex_hull_accel_control_s_axi
#(parameter
    ...
    input  wire [31:0]                   hull_size,
    input  wire                          hull_size_ap_vld,
...
localparam
    ...
    ADDR_HULL_SIZE_DATA_0 = 6'h20,
    ADDR_HULL_SIZE_CTRL   = 6'h24,
    WRIDLE                = 2'd0,
    WRDATA                = 2'd1,
    WRRESP                = 2'd2,
    WRRESET               = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg                           int_hull_size_ap_vld;
    reg  [31:0]                   int_hull_size = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_HULL_SIZE_DATA_0: begin
                    rdata <= int_hull_size[31:0];
                end
                ADDR_HULL_SIZE_CTRL: begin
                    rdata[0] <= int_hull_size_ap_vld;
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (hull_size_ap_vld)
            int_hull_size <= hull_size;
    end
end

// int_hull_size_ap_vld
always @(posedge ACLK) begin
    if (ARESET)
        int_hull_size_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (hull_size_ap_vld)
            int_hull_size_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_HULL_SIZE_CTRL)
            int_hull_size_ap_vld <= 1'b0; // clear on read
    end
end
